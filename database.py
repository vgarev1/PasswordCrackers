...
# Function to retrieve entire password history
def get_password_history():
    client, db = client_open()

    passwords_collection = db["passwords"].find({}).to_list()
...
def delete_test_passwords():
    client, db = client_open()

    db["passwords"].delete_many({"strength": "Test"})
