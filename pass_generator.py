...
class PasswordGenerator:
    ...
    def __init__(self, minlen=6, maxlen=16, minuchars=1, minlchars=1,
        minnumbers=1, minschars=1, excludeuchars="", excludelchars="",
        excludenumbers="", excludeschars="", maxAttempts=100):
        self.minlen = minlen
        self.maxlen = maxlen
        self.minuchars = minuchars
        self.minlchars = minlchars
        self.minnumbers = minnumbers
        self.minschars = minschars
        self.excludeuchars = excludeuchars
        self.excludelchars = excludelchars
        self.excludenumbers = excludenumbers
        self.excludeschars = excludeschars
    ...
    def generate(self):
        ...
        while (attempts <= self.maxAttempts):
            ...
            for password_entry in database.get_password_history():
                if ("".join(final_pass) == password_entry["password"]):
                    success = False
                    break
...
if (__name__ == "__main__"):
    # custom properties of password from using different values for __init__
    # parameters
    password = PasswordGenerator(maxlen=5, minlen=5, excludelchars="A")
